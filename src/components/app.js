...
checkLoginStatus() {
  if (this.state.loggedInStatus === "NOT_LOGGED_IN" && window.sessionStorage.getItem("token")) {
    ...
    axios
    .get(`https://letsgovocab-backend.herokuapp.com/user-by-email/${email}`)
    .then(response => {
      this.setState({
...
handleLogin(email) {
  ...
  axios.patch(`https://letsgovocab-backend.herokuapp.com/update-user-by-email/${email}`, { logged_in: "true" }, config)
  .catch(error => {
...
handleLogout() {
  ...
  axios.patch(`https://letsgovocab-backend.herokuapp.com/update-user-by-email/${email}`, { logged_in: "false" }, config)
  .catch(error => {
    console.log("Patch log status error", error)
  })
  this.setState({
